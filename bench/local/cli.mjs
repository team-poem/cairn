...
import { parseOptions, validateConfig } from "./config.mjs";
import { fixtureInfo, startFixture } from "./server.mjs";
...
if (args.length === 1 && ["--help", "-h"].includes(args[0])) {
  console.log("npm run bench:local -- <discover|replay|heal> --config FILE --runs N --engine-commit FULL_SHA --out DIRECTORY [--captures DIRECTORY]\nUse a fresh output directory. See bench/local/README.md for configuration and measurement limits.");
} else {
  try {
    const { configPath, ...options } = parseOptions(args);
    const config = validateConfig({ ...JSON.parse(await readFile(resolve(configPath), "utf8")), ...options, outputDir: resolve(options.outputDir), ...(options.captureDir ? { captureDir: resolve(options.captureDir) } : {}) });
    ...
    try {
      const report = await runBenchmark({ ...config, signal: controller.signal }, {
        ...
        fixtureInfo, startFixture, createDriver: () => new ChromeDevToolsDriver({ args: driverArgs }), createLlm, discover, runScenario, saveSkillFile,
      });
      const paths = await writeReport(report, config.outputDir);
