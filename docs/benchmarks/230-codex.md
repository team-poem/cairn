...
빈 합이며 **무료라는 뜻이 아니다**(`costComplete: false`, `comparable: false`). 아래에는 원본과 분리한 API 단가 추정액을 추가한다. 비용 교차점은 제시하지 않는다.
Claude의 제공자 보고액과 직접 비교해 가격·품질 순위를 만들지 않는다.

## 달러 환산 — Standard API 단가 추정액

README는 기존 #228 표와 같이 달러 추정액과 호출 수를 먼저 보여준다. 위 토큰 표와 원본
`230-codex.json`은 그대로 유지한다. 2026-09-11 [공식 가격표](https://developers.openai.com/api/docs/pricing)를
재확인했고, 같은 날 기록한 [#228 단가 스냅샷](228-openai-prices.json)을 재사용한다.
단위는 백만 토큰당 USD다.
...
캐시 할인은 실제 관측된 캐시 토큰에만 적용한다. Sol·Terra·Luna의 실행별 입력 합계 최댓값은
각각 85,527·85,499·74,666토큰으로, 개별 요청의 상한도 짧은 컨텍스트 기준 272,000토큰 이하다.
Standard는 CLI가 보고한 서비스 티어가 아닌 계산 가정이다. 구독에서 실제 추가 청구된
금액이 아니며 Batch·Flex·Fast·지역 할증·세금·개별 계약은 반영하지 않는다.

| 모델 | 발견 추정 USD | 복구 추정 USD | cairn 6회 추정 USD | agent 6회 추정 USD |
| --- | ---: | ---: | ---: | ---: |
...
[환산 JSON](230-codex-usd.json)은 단계별·방식별 토큰 항목과 반올림 전 추정액, 관측 파일 및 단가
스냅샷의 SHA-256을 보존한다. [환산 스크립트](230-estimate-usd.mjs)는 기존 #228 계산 방식대로
정수 나노달러로 합산하며 불완전한 사용량은 환산하지 않는다. 새 모델 호출 없이 재생성한다.
