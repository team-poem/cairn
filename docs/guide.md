...
The README is the landing page. This is the guide: how to run cairn, how to embed it, what a frozen file looks like, and how to extend the engine.
...
frozen to cart.skill.json  (replay with: cairn replay cart.skill.json)
...
That second command is your regression suite now: same input, same verdict, zero LLM calls. The report prints the proof itself (`· llm: 0 call(s)` on the CLI, `result.usage` in the library). Run it on every push for free.

## Why cairn

It sits in the gap between two things people already reach for and do not love:

| | Scripted (Playwright/Cypress) | LLM browser agents | **cairn** |
| --- | :---: | :---: | :---: |
| **Authoring** | hand-written selectors and code | plain language | **plain language** |
...
That middle column has a name now: agentic testing. Agent-authored, agent-healed, and deterministic where it counts. You do not maintain selectors, and you do not pay an LLM on every CI run. Discovery is paid once. Regression is free.
...
![The cairn loop: discover once with an LLM, freeze to plain data, replay forever for free, heal one step when the UI drifts](loop.svg)

- **discover** (LLM, once). Observes the live page, picks one action, acts, and repeats until your intent is met. Out comes a `Scenario`.
- **freeze**. That scenario is plain JSON (`*.skill.json`): a flat list of steps plus assertions, each target carrying several locators. No model, no LLM, just data.
- **replay** (no LLM). Runs the steps through a `Driver`, auto-waiting for the page to settle. A `Critic` rules on three layers of evidence: did it act, what it looked like, and the requests and console. Same input, same verdict.
- **heal** (LLM, only on a break). When a target stops resolving or the outcome diverges, the LLM maps your original step `intent` onto the new page, repairs that one step, retries, and returns a scenario to re-freeze. A green replay never calls it. The loop closes: a healed path is frozen again, so the next replay is back to zero LLM calls.

## Try it in 60 seconds

You need Node 20 or later, Chrome, and a model: an `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, or `GEMINI_API_KEY`, or no key at all with a local Claude Code or Codex CLI install. The browser is driven via Chrome DevTools MCP, launched automatically.
...
To replay an existing freeze against another origin, pair `--base-url` with `--allowed-hosts`. The [replay environment guide (한국어)](replay-environments.md) covers the library option, suite cache reuse, and temporary healing.
...
Findings are derived mechanically from each action's observation delta: failed requests, console errors, dead controls (a click that changed nothing), action errors, and slow settles. The exploring model adds problems it records itself (a confusing state, a dead end). The markdown report groups them by severity with the reproduction steps appended.

From the library, `explore(charter, { driver, llm, baseUrl })` returns an `ExploreReport`, and `renderExploreReport(report)` renders it.

## Embed it

The CLI is a convenience. The engine is the point. Author once: an AI discovers the flow, and you freeze it to a file:
...
if (healedScenario) {
  // the UI changed and cairn adapted, so write the repaired path back
...
Set a key and cairn picks the backend: Anthropic (`ANTHROPIC_API_KEY`), OpenAI (`OPENAI_API_KEY`), or Gemini (`GEMINI_API_KEY`). No key at all? A local Claude Code install (the default fallback) or the OpenAI Codex CLI (reuses your ChatGPT login) both work key-less. Force one with `createLlmClient({ backend: "codex" })` or the `CAIRN_LLM_BACKEND` env var, or implement the `LlmClient` port for any other model.

## Run a whole case list with `cairn suite`

Hand cairn your QA cases, natural-language intents plus your own success criteria, and it verifies the lot. Cached skills replay deterministically. Misses are discovered once, frozen with your criteria merged in, and replayed. The first run pays discovery. From the second run a mechanical-only case costs zero LLM calls.
...
`*.skill.json` is a flat, readable, diffable list of steps plus assertions. No code, no model:
...
A frozen destination writes `*` where the run minted the segment. `"to": "shop.co/orders/*/done"` matches one order id as well as the next, while still rejecting `/orders/999/cancel` and any other depth. A destination that would end in a wildcard is refused instead, because `/products/*` is reached by that app's own `/products/login`. Scenarios that use the notation carry `"wildcards": true`. Without it a `*` is matched as the literal character it was frozen as.

Each `target` keeps several locators. `text` (accessible name) comes first, with `nth` to address the Nth of several identically-named elements (`{"text": "Accept", "role": "button", "nth": 2}` is the 3rd Accept button, 0-based). `role` plus `index` is a rename-resilient fallback, and `selector` is a CSS escape hatch. This is what lets replay survive a redesign without falling back to the LLM.

A step's `expect` is its post-condition. Replay waits for it deterministically (so an async submit is not raced) and only heals if it truly diverges.
...
A frozen scenario is the test as data, and a run emits the execution as data. Pass a `TraceSink` (the `trace` option on `runScenario` and `runSuite`) and the engine streams its whole lifecycle as one flat, ordered, versioned sequence of events. Watch it live, or store it and open it in a viewer later:
...
Nothing is silent and nothing is overwritten. The trace records what the model decided (and why), what a gate refused (a policy block, an ambiguity it would not guess through), every assertion labeled by who wrote it (`origin: user | derived`) and who judged it (`checkedBy: code | model`), and every repair as what broke and what it became. Repairs are judged against the original assertions, so a heal can never move the goalposts. Without a sink, no events are even built. A sink that throws can never change a verdict. The contract is versioned and lives at [`spec/core/trace.md`](../spec/core/trace.md).

Storing it is one import. `JsonlTraceSink` appends each event as a line of JSONL (a failed write is counted, never thrown, because recording must not change the verdict), and a step's screenshot rides along as a sidecar keyed by the event's own `seq`. Even a run that dies mid-way leaves everything already written readable.

## When the UI changes: self-heal

Say a redesign renames the login button from `Log in` to `Sign in`. A scripted test goes red until someone fixes the selector. cairn notices the frozen target no longer resolves, asks the LLM to map the step's recorded intent onto the new page, repairs that one step, and finishes the run:
...
self-healed 1 step(s):
  · "Log in" became "Sign in"
  re-frozen to cart.skill.json
```

The repaired path is written back. The next replay is deterministic again, with zero LLM calls. Healing is surgical: one step, judged against your original assertions, so a heal can never turn a genuinely broken flow green.

## How it works: one pipeline, seven ports

The execution body is a five-stage pipeline. No environment- or domain-specific logic lives inside it. Every variable behavior arrives through an interface.

```
Context ─► Plan ─► Execute ─► Judge ─► Report
```

- **Context** assembles grounding (the intent, and later a git diff, a ticket, docs)
...
- **Judge** is a `Critic` ruling on three layers of evidence, not a screenshot guess: `execution` (did it act or navigate), `perception` (what it looked like), `logic` (requests, console)
- **Report** emits the result anywhere (console, JSON, your tracker)

The seven extension points, `ContextProvider`, `Planner`, `Driver`, `SkillStore`, `Critic`, `Reporter`, and `TraceSink`, are how you adapt cairn without forking it. The LLM lives behind its own seam (`LlmClient`), so neither a model nor a browser is hard-wired into the core. Where a whole port is too much, `custom` assertions and `actions` let a product define its own success criteria and interactions inline. The engine ships defaults, and your product defines the specifics.

Two design lines hold the whole thing together:

- **Replay is deterministic.** A frozen scenario replays with no LLM in the loop. The LLM is summoned only to discover a new scenario or to self-heal a broken one.
...
cairn is made to be built on, not scattered across your service as test code. A few things it powers:

- **A QA tool.** Non-developers write flows in plain language, then watch them replay and self-heal.
- **A CI regression gate.** Frozen flows run on every PR, and drift heals instead of going red.
- **A synthetic monitor.** Replay critical paths against production and alert only when one truly breaks.
- **A visual-replay app.** The engine streams per-step progress and screenshots, and you draw the UI.
...
Discovery is authoring, not running. It happens once, and what it produces is plain JSON you can read, diff, and edit before you commit it. Frozen assertions are grounded in what the run actually observed (a hallucinated check is dropped). A scenario with nothing to verify fails closed. A run that blocks partway fails even when the executed prefix's assertions held. Steps the agent guessed (blind key chains instead of a resolved target) are flagged at freeze time.
...
Name the file that embeds cairn and calls `runScenario(...)` `*.agentic.ts`, paired with its frozen `*.skill.json`:
...
The suffix is intentionally distinct from `*.test.ts` and `*.spec.ts` (ordinary unit and e2e tests). It marks the agentic tier and gives tooling a stable glob: `**/*.agentic.ts`.

## Structure

Ports and adapters (hexagonal). `core/` is the pure domain and the ports, and `adapters/` implements them. Dependencies point inward: adapters depend on core, never the reverse.
...
`cairn-engine` is on npm. The full loop (discover, freeze, replay, self-heal) works today and is benchmarked: deterministic, LLM-free replay on real multi-step flows, discovery paid once (about $0.5) against $0 replays, and survival across UI renames without re-reasoning. Multiple LLM backends (Anthropic, OpenAI, Gemini, plus key-less Claude Code and Codex CLI) and a browser and extension entry ship in the box. Every result carries its own cost proof. `result.usage` counts LLM calls exactly (a clean replay reports `llmCalls: 0`) and token totals whenever the backend measures them.

What's next sits above the engine: input sources (git diff and ticket `ContextProvider`s), and a first-party surface for visual replay is planned. The engine stays the product. Whatever runs it is just its first consumer, and the interfaces are the contract.
