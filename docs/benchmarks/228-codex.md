...
Codex는 시도별 관측 기록을 사용한다. Claude 작성자는 재생 5회가 모두 0호출이라고도 보고했으므로 첫 탐색의 7회와 이후
평평한 재생 선은 그 보고에서 도출한다. 매번 탐색의 중간 실행별 값은 알 수 없으므로
0회 기준점과 6회 누적 42회만 점으로 표시하고 점선으로 연결한다. 이 점선은 중간 값을
측정했다는 뜻이 아니며 그래프 안에도 이를 표시한다. Codex는 각 실행의 실제 누적
관측값을 실선과 점으로 표시한다.
