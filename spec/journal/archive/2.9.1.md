...
Archived 2026-09-11. 2 entries from the 2.9.1 cycle.

## Release 2.9.1

1 pull request since 2.9.0: #234.

131 files changed, 2496 insertions(+), 3964 deletions(-).

The journal stopped being a set of rules people had to remember and became part of the harness.
Entries are append-only and machine-readable, status is derived rather than stored, the cycle folds
into one archive at release, and English is checked instead of agreed. `AGENTS.md`, `CONTRIBUTING.md`
and `state.md` are rewritten in English, and 2.3.0 through 2.9.0 are archived as English summaries.

Nothing in the engine changed. `packages/harness/` is untouched, the trace contract stays at 1.6, and
no published behaviour differs from 2.9.0. This release exists so that the version carrying the
release workflow reaches the default branch, which is what a `workflow_dispatch` needs before it can
be run at all.

---

## The journal became append-only, English is enforced in CI, and state.md stopped being a status board
*PR #234*
