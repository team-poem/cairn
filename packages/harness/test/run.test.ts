...

{
  ...
  {
    it("traceHeaderVersionLiteral: the header says version 1.4 literally (spec/core/trace.md) and carries a UUID runId", async () => {
      const events: TraceEvent[] = [];
      await runScenario(traceScenario, { driver: new FakeDriver({ evidence: traceEvidence }), reporter: silent, trace: { emit: (e) => events.push(e) } });
      ...
      expect(header.payload.version).toBe("1.4");
