...
- **다음 사이클(피벗 — #158 닫음, 디스커션 #168):** first-party 러너 중단(데스크탑 러너 범용화로 뷰어는
  ...
  **진행(2026-09-07):** 종결 = #199(PR #202) · #200(PR #201, `urlMatchesFrozen` 쿼리 부분집합 + `hasStablePath`
  ...
  공개 어휘냐"라 이슈 198의 두 질문 답 대기 → PR #221로 재제출, 리뷰에서 차단 3건: 인지 캡처가 verbose
  트리를 드라이버 캐시에 남겨 freeze와 replay가 다른 트리를 센다 · 같은 캐시가 custom dropdown `select`를
  깨뜨린다 · `perceive` 훅이 이름을 다듬거나 role을 고치면 관찰 생성자가 루프 밖에서 던져 탐색 전체가 죽는다).
  #214는 PR #223으로 완료(`cost` 모드 — agent/cairn 두 갈래, 실행별 버전 일정, 예약 포트로 replayEnvironment
  없이 heal 재동결, 교차점은 측정이 아닌 경우 내지 않음). **#225 신규(차단):** 두 루프가 페이지 요소 목록을
  `(unchanged from previous step)`으로 생략하는데 `LlmClient`는 대화가 없어 모델이 그 턴을 본 적이 없다 →
  한 동작으로 끝나는 흐름만 탐색된다. PR #226. 순서 = #226 → #221(그 위에서 리베이스) → #170 벤치 PR. #196·#175는 2.10.0. #184 advisory→fail-closed 전환은 #169 벤치 뒤; #169는 amazon의 "구조 수정 더 기다려야 하나" 질문에 "반대,
...
- **비용 축 실측(2026-09-09, 로컬 픽스처·소넷5·6실행):** 이동 단계에서 매번 탐색 $0.124 대 한 번 탐색 후
  재생 $0.021, 교차점 2번째 실행. 모델별 분리 기준이며 CLI가 배경에 쓰는 haiku 몫은 별도로 나온다.
  구독으로 돌려도 제공자가 `costBasis: "list"`로 정가 계산해 돌려주므로 API 환산이 곧 이 값이다.
  폼·상태 단계는 #225 때문에 12/12 실패라 수치 없음.
