...
- **메인테이너 2차 코멘트:** ① **충돌 판정이 얼릴 method를 무시했다** — `POST /api/checkout` + 
  `GET /api/checkout/status`처럼 **읽기가 쓰기용 체크를 "소비"한 것으로 오판**해 드롭했다. 얼리는 체크가
  POST에 묶이면 GET은 그걸 만족시킬 수 없으므로 충돌이 아니다. 충돌 판정을 **얼릴 method 기준**으로 옮겼다
  (method 계산을 충돌 검사 앞으로). 테스트 2개로 고정: 다른 method 형제는 통과, 같은 method 형제는 드롭.
  ② `stableQuerySuffix` 주석이 "파라미터 순서를 바꾸는 클라이언트는 쿼리를 안 얼린다"고 했는데,
  **프리즈는 URL 하나만 보므로 그걸 알 수 없다.** 사실대로 고쳤다.
- **판단 대기(메인테이너 질문):** 드롭이 **영구 빨강 스킬**을 만들 수 있다. 재현 확인 — SPA 장바구니처럼
  드롭 후 가드 둘만 남으면 `markVacuous`가 둘 다 찍고 전부-항진 fail-closed로 죽으며, 사유도
  "모든 단언이 이미 만족돼 있었다"라 진짜 원인을 말하지 않는다. 선택지가 갈린다 —
  (a) **마지막 판별력 있는 체크면 드롭하지 않는다**(약한 체크를 얼린다: 메인테이너 문구 그대로지만
  같은 코멘트의 첫 요청과 상충), (b) 드롭은 유지하되 **#184의 `unprovenAction`이 이 경우도 덮게 해서
  사유를 정직하게** 만든다(여전히 빨강이지만 fail-closed 계보와 일관). 메인테이너 답을 받아 결정한다.
