...
export function renderCostMarkdown(report) {
  ...
  const sections = report.summaries.map((summary) => {
    ...
    // What discovery cost and what a repair cost are two answers (#230), so they are printed apart.
    // A repair line counts attempts, and says how many came back re-frozen, with the money spent on
    // all of them; a repair that could not be verified still paid.
    const phaseCost = (phase) => !summary.costMeasured ? "cost not measured" : phase.costUsd === null ? "cost unknown" : money(phase.costUsd);
    const phases = arms.flatMap(([name, arm]) => arm.phases ? [
      `${name} · discovery: ${arm.phases.discovery.runs} run(s), ${arm.phases.discovery.calls} call(s), ${arm.phases.discovery.tokens} tokens, ${phaseCost(arm.phases.discovery)}`,
      `${name} · repair: ${arm.phases.repair.runs} replay run(s) called the model, ${arm.phases.repair.refrozen} re-frozen, ${arm.phases.repair.calls} call(s), ${arm.phases.repair.tokens} tokens, ${phaseCost(arm.phases.repair)}`,
      `${name} · replay without a call: ${arm.phases.replay.passed} of ${arm.phases.replay.runs} passed`,
    ] : []);
    // One call can bill more than one model, so the split says what the model under test cost on
    // its own, next to the helper model the tool runs for itself.
    const split = arms.flatMap(([name, arm]) => Object.entries(arm.models ?? {}).map(([id, model]) =>
      `${name} · ${cell(id)}: ${model.costComplete ? money(model.costUsd) : "unknown"}${model.listPriced ? " at list price" : " (not list price)"}, ${model.calls} call(s), ${BILLED_TOTAL(model)} tokens`));
    return `### ${cell(summary.tier)}\n\nCrossover: ${crossover}.\n${[...notes, ...phases, ...split].map((note) => `- ${cell(note)}`).join("\n")}\n\n${header}\n${divider}\n${rows.join("\n")}`;
  });
  return `# Local cost comparison\n\n${report.incomplete ? "INCOMPLETE: " + cell(report.stopReason) : "Requested attempts completed."}\n\nEngine: ${cell(report.engine.version)} at ${cell(report.engine.commit)} (dirty: ${report.engine.dirty}).\nBuild SHA-256: ${cell(report.engine.buildHash)}.\nConfiguration SHA-256: ${report.configHash}.\nMeasured: ${report.startedAt} → ${report.finishedAt}.\nLLM source: ${cell(report.configuration.llm?.source ?? "unknown")}${report.configuration.llm?.source === "llm" ? ` (${cell(report.configuration.llm.backend)}: ${cell(report.configuration.llm.model)})` : ""}.\n\n${sections.join("\n\n")}\n\nThe agent arm discovers on every run; the cairn arm discovers once and then replays, healing when the fixture version changes under it and carrying the repair forward. Both arms meet the same version on the same run. A discover run fails when no replayable scenario came back or the fixture never completed; a replay run additionally has to satisfy the frozen assertions, so the two failure counts are not the same measurement. Cost and tokens are running totals for that arm up to and including the run, not that run alone. A token total counts every billed field, cache creation included, and reads as a lower bound once a call reported no usage or reported only some of those fields; it is a count, not a price. Cost is what the provider reported, not a billing statement, and a threshold can be exceeded by the final call. An arm's cost is everything the run spent; where a line says a model was priced at list, that model's share is what an API caller would have paid at published rates for the same tokens.\n\nA crossover says only that this schedule, on these fixtures, reached the point where discovering once had cost less. It is not a general saving; how often an application breaks a freeze is the variable, and this fixes it by construction.\n${report.budget && report.configuration.llm?.source === "llm" ? `\nReported provider cost: ${report.budget.costComplete ? money(report.budget.measuredCostUsd) : "unknown"}; cost complete: ${report.budget.costComplete}; calls: ${report.budget.calls}${report.budget.stopReason ? `; stopped: ${cell(report.budget.stopReason)}` : ""}.\n` : ""}`;
}
