  ...
  const sections = report.summaries.map((summary) => {
    ...
    const rows = Array.from({ length: summary.runs }, (_, index) => {
      const cells = arms.map(([, arm]) => {
        ...
        return `${point.calls} | ${point.tokens} | ${cost}`;
      });
