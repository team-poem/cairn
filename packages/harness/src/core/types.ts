...
export interface Scenario {
  ...
  wildcards?: true;
  /** Set by discover when the run performed an action no check could be frozen for — the proving
   * request's URL has no stable path to check (`POST https://api.shop.co/`, or a run-minted first
   * path segment), so any check written from it would be satisfied by every request to that host.
   * Replay fails closed on it (spec/core/judgment.md), because the remaining assertions can be
   * satisfied without the action ever firing. Absent whenever a proof was frozen. */
  unprovenAction?: true;
}
