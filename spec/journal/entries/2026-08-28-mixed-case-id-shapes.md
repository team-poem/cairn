...
- **2차 교차검증 대응(HIGH 0, MAJOR 2):** ① **런 ≥2 조건이 사간 대가** — 숫자가 한 덩어리인 id
  (`ORD12345678`·`INV20260827`·`TXN0001234567`)는 밀도가 73%든 77%든 안 잘렸다. 주문·송장·거래번호가
  정확히 이 모양이라 사람이 읽는 id 중 가장 흔하다. 리뷰어 제안대로 **런이 1개여도 밀도 60% 이상이면
  id로 본다**를 추가했다. 이 문턱은 지키려는 표준 이름 전부(X25519Key 56%·Ed25519Sign 45%·
  ISO8601Date 36%·SHA256Digest 25%) 위에 선다. 이 분기는 대문자를 요구하지 않으므로
  소문자 `ord12345678`도 잡힌다(`sha256sum` 33%·`base64decode` 17%는 문턱 아래).
  ② **KNOWN FALSE POSITIVE 문구가 실제보다 좁았다** — "변환기 이름"이라고 적었지만 `P256AndP384`처럼
  **표준 둘을 나열한 이름**도 같이 잘린다. 런 규칙은 표준 하나는 지키지만 둘은 못 지킨다.
  문구를 넓히고 `IPv4ToIPv6`(정확히 밀도 문턱 위)과 PascalCase 네임스페이스(JWT로 오인)를
  반례로 추가했다. 20% 인하의 대가가 `IPv4ToIPv6` 하나라는 것도 표에 드러난다. #178 본문의 "대문자 포함 id" 한계 해소. substring 표현형에서 오는 문제
