...
const input = await readFile(new URL("./230-codex.json", import.meta.url));
...
const estimates = observations.measurements.filter((measurement) => measurement.kind === "cost-comparison").map((measurement) => {
  ...
  return {
    ...
    phases: Object.fromEntries(["discovery", "repair", "replay"].map((phase) => [phase, summarize(measurement.records.filter((record) => record.arm === "cairn" && (record.action === "discover" ? "discovery" : record.observedUsage.llmCalls > 0 ? "repair" : "replay") === phase))])),
...
await writeFile(new URL("./230-codex-usd.json", import.meta.url), JSON.stringify({
